...
class BacktestVisualizer:
    ...
    def plot_backtest_results(self, df, trades_df=None, title="Backtest Results"):
        ...
        ax1.plot(df['timestamp'], df['close'], label='Close Price', linewidth=1)
        ...
        ax1.set_title(f'{title} - Price and Trading Signals', fontsize=12, fontweight='bold')
        ax1.set_xlabel('Date')
        ...
        if trades_df is not None and len(trades_df) > 0:
            ...
            bars = ax5.bar(range(len(pnl_pct)), pnl_pct, color=colors, alpha=0.6)
            
            ax5.set_title('Individual Trade P&L (%)', fontsize=12, fontweight='bold')
    ...
    def plot_strategy_comparison(self, results_dict):
        ...
        for name, (df, metrics) in results_dict.items():
            ax1.plot(df['timestamp'], df['total_value'], label=name, linewidth=2)
    ...
    def plot_heatmap(self, df, indicator='RSI'):
        ...
        if indicator in df.columns:
            ...
            ax.set_title(f'{indicator} Heatmap by Hour and Day', fontweight='bold')
            ax.set_xlabel('Date')
...
def main():
    """Programme principal pour exécuter le backtest"""
    
    print("="*60)
    print("🤖 CRYPTO TRADING BOT - BACKTESTING SYSTEM")
    print("="*60)
    ...
    # Parameters de risque
    STOP_LOSS = 0.02  # 2%
    TAKE_PROFIT = 0.05  # 5%
    POSITION_SIZE = 0.5  # Utiliser 50% du capital par trade
    ...
    data = dm.prepare_data_for_backtesting(data)
    
    # 3. Définir les stratégies à tester
    strategies = {
        'RSI Strategy': RSIStrategy({'rsi_period': 14, 'rsi_oversold': 30, 'rsi_overbought': 70}),
        'MACD Strategy': MACDStrategy({'macd_fast': 12, 'macd_slow': 26, 'macd_signal': 9}),
        'Bollinger Bands': BollingerBandsStrategy({'bb_period': 20, 'bb_std': 2}),
        'MA Cross': MovingAverageCrossStrategy({'ma_fast': 20, 'ma_slow': 50, 'ma_type': 'EMA'}),
    ...
    for name, strategy in strategies.items():
        ...
        try:
            ...
            results[name] = (df_results, metrics)
    ...
    # 5. Trouver la meilleure stratégie
    print("\n🏆 Best Strategy:")
    best_strategy = max(results.items(), 
                       key=lambda x: x[1][1]['total_return_pct'])
    best_name = best_strategy[0]
    best_metrics = best_strategy[1][1]
    
    print(f"  Winner: {best_name}")
    backtester.print_performance_summary(best_metrics)
    
    # 6. Sauvegarder les résultats
    print("\n💾 Saving results...")
    for name, (df_results, metrics) in results.items():
        filename = f"{name.replace(' ', '_')}_{SYMBOL}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        dm.save_backtest_results(metrics, filename)
        
        # Sauvegarder le journal des trades
        trades_df = backtester.get_trade_log()
    ...
    # Graphique pour la meilleure stratégie
    best_df = best_strategy[1][0]
    trades_df = backtester.get_trade_log()
    
    fig1 = visualizer.plot_backtest_results(
        best_df, 
        trades_df, 
        title=f"Best Strategy: {best_name}"
    )
    plt.savefig(f'backtest_results/best_strategy_{SYMBOL}.png', dpi=100, bbox_inches='tight')
    
    # Comparaison des stratégies
    fig2 = visualizer.plot_strategy_comparison(results)
    plt.savefig(f'backtest_results/strategy_comparison_{SYMBOL}.png', dpi=100, bbox_inches='tight')
